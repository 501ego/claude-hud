...
function computeCompactEta(stdin: Awaited<ReturnType<typeof readStdin>>): number | null {
...
function computeFiveHourExhaust(usageData: RenderContext["usageData"]): number | null {
  if (!usageData || usageData.fiveHour === null) return null;
  ...
  if (!Number.isFinite(exhaustMin)) return null;

  if (usageData.fiveHourResetAt) {
    const resetMin = (usageData.fiveHourResetAt.getTime() - Date.now()) / 60000;
    if (resetMin > 0 && exhaustMin >= resetMin) return null;
  }

  return Math.round(exhaustMin);
}

function readEffortLevel(): string | null {
  try {
    const settingsPath = join(getClaudeConfigDir(getHomeDir()), "settings.json");
    const raw = JSON.parse(readFileSync(settingsPath, "utf8")) as { effortLevel?: unknown };
...
export async function main(overrides: Partial<MainDeps> = {}): Promise<void> {
  ...
  try {
    ...
    const fetchGitStatus = deps.getGitStatus === getGitStatus
      ? (cwd?: string) => getGitStatusCached(cwd, getGitStatus)
      : deps.getGitStatus;
    ...
    let burnTrend: number[] | null = null;
    ...
    let pet: RenderContext["pet"] = null;
...
function writeUsageState(
  ...
  try {
    ...
    const merged = new Map<string, UsageWindowEntry>();
    const prevWindows = Array.isArray(existing.windows) ? (existing.windows as UsageWindowEntry[]) : [];
    for (const w of prevWindows) {
      if (!w || (w.kind !== '5h' && w.kind !== '7d') || typeof w.percent !== 'number') continue;
      merged.set(`${w.kind}|${w.resetAt ?? ''}`, w);
    }

    const current: Array<{ kind: '5h' | '7d'; percent: number | null; resetAt: string | null }> = [
    ...
    for (const c of current) {
      ...
      if (prev && prev.percent === c.percent) continue;
