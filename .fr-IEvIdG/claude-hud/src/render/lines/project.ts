...
import { git as gitColor, gitBranch as gitBranchColor, warning as warningColor, critical as criticalColor, label, model as modelColor, project as projectColor, red, green, yellow, dim, custom as customColor, effortCurve } from '../colors.js';
...
export function renderProjectLine(ctx: RenderContext): string | null {
  ...
  if (display?.showModel !== false) {
    const currentModel = formatModelName(getModelName(ctx.stdin), ctx.config?.display?.modelFormat, ctx.config?.display?.modelOverride);
    const providerLabel = getProviderLabel(ctx.stdin);
    const currentDisplay = providerLabel ? `${currentModel} | ${providerLabel}` : currentModel;
    let badge = modelColor(`[${currentDisplay}]`, colors);
    if (display?.showEffort === true && ctx.effortLevel) {
      const curve = effortCurve(ctx.effortLevel);
      if (curve) badge += ` ${dim(curve)}`;
    }
    parts.push(badge);
  }
