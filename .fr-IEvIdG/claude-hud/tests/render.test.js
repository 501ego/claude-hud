...
test('renderSessionLine includes duration and formats large tokens', () => {
  ...
  const line = renderSessionLine(ctx);
  assert.ok(line.includes('1m'), 'expected session duration in the line');
...
test('label color overrides apply across shared secondary text surfaces', () => {
  ...
  ctx.sessionDuration = '1m';
  ctx.transcript.modelUsage = {
    'claude-opus-4-8': { inputTokens: 1000, outputTokens: 500, cacheCreationTokens: 0, cacheReadTokens: 0 },
  };

  const expected = '\x1b[38;2;171;205;239m';
  assert.ok(renderIdentityLine(ctx).includes(`${expected}Context\x1b[0m`));
  // Cost/tokens moved to the session element; its "Session" label uses the label color surface.
  assert.ok(renderSessionStatsLine(ctx)?.includes(`${expected}Session\x1b[0m`));
...
test('renderSessionLine shows an estimated cost from context-window usage when showCost is enabled', () => {
  const ctx = baseContext();
  ctx.config.display.showCost = true;
  ctx.stdin.model = { id: 'claude-opus-4-8' };
  ctx.stdin.context_window.current_usage = {
    input_tokens: 100000,
    output_tokens: 50000,
  ...
  // 100000*5 + 50000*25 + 20000*0.5 + 10000*6.25 = 1,822,500 → $1.82
  const line = stripAnsi(renderSessionLine(ctx));
  assert.ok(line.includes('$1.82'), `expected estimated session cost, got: ${line}`);
});
...
test('renderUsageLine shows 7d reset countdown in bar mode when above threshold', () => {
  ...
  const line = stripAnsi(renderUsageLine(ctx));
  assert.ok(line.includes('[5h]'), `should include 5h label in bar mode: ${line}`);
...
test('renderUsageLine shows weekly-only usage without a ghost 5h section', () => {
  ...
  const line = stripAnsi(renderUsageLine(ctx));
  assert.ok(line.includes('[wk]'), `should render the weekly usage window: ${line}`);
  assert.ok(!line.includes('[5h]'), `should not render a ghost 5h section: ${line}`);
  assert.ok(line.includes('13%'), `should render the weekly percentage: ${line}`);
...
test('renderUsageLine uses custom usage palette overrides', () => {
  ...
  assert.ok(line, 'should render usage line');
  assert.ok(line.includes('\x1b[36m⣿⣿'), `expected custom usage braille bar color, got: ${JSON.stringify(line)}`);
  assert.ok(line.includes('\x1b[36m25%\x1b[0m'), `expected custom usage percentage color, got: ${JSON.stringify(line)}`);
  assert.ok(line.includes('\x1b[35m⣿⣿⣿⣿⣿⣿'), `expected custom usage warning braille bar color, got: ${JSON.stringify(line)}`);
...
test('render expanded layout honors custom elementOrder including activity placement', () => {
  ...
  const combinedIndex = lines.findIndex(line => line.includes('[5h]') && line.includes('Context'));
  ...
  // Project is combinable now, so it greedily packs onto the same line as usage/context.
  assert.equal(projectIndex, combinedIndex, 'project should share the combined usage/context line');
...
test('render expanded layout combines usage and context when adjacent in elementOrder', () => {
  ...
  assert.ok(lines[0].includes('[5h]'), 'combined line should include usage window');
...
test('render expanded layout keeps usage and context separate when not adjacent', () => {
  ...
  // Memory is not a combinable element, so it interrupts the usage/context run.
  ctx.memoryUsage = {
    totalBytes: 16 * 1024 ** 3,
    usedBytes: 10 * 1024 ** 3,
    freeBytes: 6 * 1024 ** 3,
  ...
  ctx.config.display.showMemoryUsage = true;
  ctx.config.elementOrder = ['usage', 'memory', 'context'];
  ...
  const combinedLine = lines.find(line => line.includes('[5h]') && line.includes('Context'));

  assert.ok(usageLine, 'usage should render on its own line');
  assert.ok(contextLine, 'context should render on its own line');
  assert.equal(combinedLine, undefined, 'usage and context should not combine when separated by a non-combinable element');
});
