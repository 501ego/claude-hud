...
test("CLI renders expected output for a basic transcript", async (t) => {
  ...
  // The pet animates off the wall clock, which would make the snapshot
  // nondeterministic — disable it for this test (it has its own suite).
  const hudCfgDir = path.join(homeDir, ".claude", "plugins", "claude-hud");
  await import("node:fs/promises").then((fs) =>
    fs.mkdir(hudCfgDir, { recursive: true }),
  );
  await writeFile(
    path.join(hudCfgDir, "config.json"),
