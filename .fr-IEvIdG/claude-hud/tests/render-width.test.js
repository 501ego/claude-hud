...
// Claude Code captures statusline stdio and communicates the real terminal
// width via $COLUMNS (v2.1.153+), so COLUMNS outranks stderr.columns.
test('render prefers COLUMNS over stderr.columns when stdout.columns is unavailable', () => {
  const ctx = baseContext();
  const originalEnvColumns = process.env.COLUMNS;
  ...
  assert.ok(lines.every(line => displayWidth(line) <= 10), 'COLUMNS width should be honored over stderr');
});
...
test('render falls back to a safe default width when no terminal size is available', () => {
  ...
  assert.ok(lines.every(line => displayWidth(line) <= 100), 'all lines should fit the safe fallback width (UNKNOWN_TERMINAL_WIDTH=100)');
});
...
test('render keeps context and usage as separate lines when a narrow terminal cannot fit both', () => {
  ...
  assert.ok(lines.some(line => line.includes('[5h]')), 'usage line should remain visible');
