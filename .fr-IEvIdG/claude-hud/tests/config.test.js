...
test('loadConfig returns valid config structure', async () => {
  ...
  // Note: loadConfig() reads the machine's real config.json, so elementOrder may be
  // a user-customized subset. The dedicated mergeConfig({}) tests assert the default.
  ...
  assert.equal(typeof config.display.showBurnHeat, 'boolean');
  assert.equal(typeof config.display.showCacheGlyph, 'boolean');
  assert.equal(typeof config.display.showAutocompactMarker, 'boolean');
  assert.equal(typeof config.display.showSparkline, 'boolean');
  assert.equal(typeof config.display.showApiEquivCost, 'boolean');
  ...
  for (const key of ['context', 'usage', 'warning', 'usageWarning', 'critical', 'model', 'project', 'git', 'gitBranch', 'label', 'custom', 'burnLow', 'burnHigh']) {
...
test('mergeConfig defaults colors to expected semantic palette', () => {
  const config = mergeConfig({});
  assert.equal(config.colors.context, '#f4a7b9');
  assert.equal(config.colors.usage, '#c9a0c0');
  assert.equal(config.colors.warning, '#f5c87a');
  assert.equal(config.colors.usageWarning, '#e8a0b4');
  assert.equal(config.colors.critical, '#f07090');
  assert.equal(config.colors.model, '#f4a7b9');
  assert.equal(config.colors.project, '#f9c4d4');
  assert.equal(config.colors.git, '#e8a0b4');
  assert.equal(config.colors.gitBranch, '#d98aa0');
  assert.equal(config.colors.label, 'dim');
  assert.equal(config.colors.custom, 208);
  assert.equal(config.colors.tools, '#a8d4f5');
});
