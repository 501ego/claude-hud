...
interface BurnSampleFile {
  samples: BurnSample[];
  updatedAt: string;
}

function getBurnSamplePath(transcriptPath: string): string {
  const hash = createHash('sha256').update(path.resolve(transcriptPath)).digest('hex');
  return path.join(getHudPluginDir(getHomeDir()), 'burn-samples', `${hash}.json`);
}

function loadInWindowSamples(transcriptPath: string): BurnSample[] | null {
  const filePath = getBurnSamplePath(transcriptPath);
  let samples: BurnSample[];
  try {
    const raw = JSON.parse(readFileSync(filePath, UTF8)) as BurnSampleFile;
  ...
  if (monotone.length < 2) return null;
  return monotone;
}

export function readBurnSamples(transcriptPath: string): number | null {
...
export function readBurnTrend(transcriptPath: string): number[] | null {
  const inWindow = loadInWindowSamples(transcriptPath);
  if (!inWindow) return null;
  const series: number[] = [];
...
export function writeBurnSamples(transcriptPath: string, tokensTotal: number): void {
  const filePath = getBurnSamplePath(transcriptPath);
  try {
    ...
    const data: BurnSampleFile = { samples, updatedAt: new Date().toISOString() };
