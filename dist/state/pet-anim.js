...
function keyFor(transcriptPath) {
    return createHash('sha256').update(path.resolve(transcriptPath)).digest('hex').slice(0, 16);
}
function loadFile() {
    try {
        const raw = JSON.parse(fs.readFileSync(getAnimPath(), 'utf8'));
...
export function readPetAnim(transcriptPath) {
    const entry = loadFile().entries[keyFor(transcriptPath)];
    if (!entry || typeof entry.offset !== 'number' || typeof entry.atMs !== 'number')
        return null;
    return entry;
}
export function writePetAnim(transcriptPath, anim) {
    try {
        const file = loadFile();
        file.entries[keyFor(transcriptPath)] = anim;
        for (const [key, entry] of Object.entries(file.entries)) {
            if (anim.atMs - entry.atMs > ANIM_RETENTION_MS)
...
//# sourceMappingURL=pet-anim.js.map