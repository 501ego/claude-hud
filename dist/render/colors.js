...
const EIGHTH_BLOCKS = ['', '▏', '▎', '▍', '▌', '▋', '▊', '▉'];
export function quotaHeatBar(percent, width = 10, colors) {
...
export function quotaBrailleBar(percent, width = 8, colors) {
...
export function burnHeatColor(velocity, colors) {
    if (velocity >= 2000)
        return resolveAnsi(colors?.burnHigh, '\x1b[38;2;240;112;144m');
    if (velocity >= 500)
        return resolveAnsi(colors?.usage, MAGENTA);
    return resolveAnsi(colors?.burnLow, '\x1b[38;2;244;167;185m');
}
export function coloredBarWithMarker(percent, width = 10, markerPercent, burnColor, colors) {
    const safeWidth = Number.isFinite(width) ? Math.max(0, Math.round(width)) : 0;
    const safePercent = Number.isFinite(percent) ? Math.min(100, Math.max(0, percent)) : 0;
    const filled = Math.round((safePercent / 100) * safeWidth);
