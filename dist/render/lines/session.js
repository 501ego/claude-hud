...
function seriesToSparkline(series) {
    const min = Math.min(...series);
    const max = Math.max(...series);
    const range = max - min;
    const normalize = (v) => range === 0 ? BRAILLE_LEVELS.length - 1 : Math.round(((v - min) / range) * (BRAILLE_LEVELS.length - 1));
    const cols = 4;
    const sampled = [];
...
export function renderSessionStatsLine(ctx) {
    ...
    if (display?.showUsageForecast !== false
