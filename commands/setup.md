...
4. Install the launcher. It lives outside the versioned plugin directory and resolves the newest installed version at every render, so plugin updates never break the statusline and no shell pipeline runs per render:
   ```bash
   mkdir -p "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/scripts"
   cp "{PLUGIN_DIR}/scripts/claude-hud-launcher.mjs" "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/scripts/claude-hud-launcher.mjs"
   ```
   (`{PLUGIN_DIR}` is the path found in step 1.)

5. Generate command. `{CLAUDE_DIR}` is the absolute expansion of `${CLAUDE_CONFIG_DIR:-$HOME/.claude}` (quotes around paths handle spaces):

   **When runtime is bun** - add `--env-file /dev/null` to prevent Bun from auto-loading project `.env` files:
   ```
   "{RUNTIME_PATH}" --env-file /dev/null "{CLAUDE_DIR}/scripts/claude-hud-launcher.mjs"
   ```

   **When runtime is node**:
   ```
   "{RUNTIME_PATH}" "{CLAUDE_DIR}/scripts/claude-hud-launcher.mjs"
   ```
...
3. Install the launcher (resolves the newest installed version at every render — plugin updates never break the statusline, and no PowerShell process spawns per render):
   ```powershell
   $claudeDir = if ($env:CLAUDE_CONFIG_DIR) { $env:CLAUDE_CONFIG_DIR } else { Join-Path $HOME ".claude" }
   New-Item -ItemType Directory -Force (Join-Path $claudeDir "scripts") | Out-Null
   Copy-Item (Join-Path "{PLUGIN_DIR}" "scripts/claude-hud-launcher.mjs") (Join-Path $claudeDir "scripts/claude-hud-launcher.mjs") -Force
   ```
   (`{PLUGIN_DIR}` is the path found in step 1.)

4. Generate command. `{CLAUDE_DIR}` is the resolved `$claudeDir` absolute path (forward slashes are fine and avoid escaping issues):

   **When runtime is bun** - add `--env-file NUL` to prevent Bun from auto-loading project `.env` files:
   ```
   "{RUNTIME_PATH}" --env-file NUL "{CLAUDE_DIR}/scripts/claude-hud-launcher.mjs"
   ```
...
try:
    ...
    data = {}
data['statusLine'] = {'type': 'command', 'command': r'GENERATED_COMMAND_PLACEHOLDER', 'refreshInterval': 1}
...
**`refreshInterval: 1` is required**: it makes Claude Code re-run the statusline every second so the HUD pet animates while idle (patrol, blinking). Without it the statusline only refreshes on session events.
...
The HUD ships with a pixel-art pet enabled by default: it patrols the statusline, reacts to session telemetry (working, errors, context pressure, quota alerts) and levels up with lifetime token usage. Two styles exist: `cat` (default) and `claude` (Clawd, the terracotta CLI mascot).

Use AskUserQuestion:
- header: "Pet"
- question: "The HUD includes an animated pet (enabled by default). Which style do you want?"
- options:
  - "Cat (default)" — british shorthair pixel cat
  - "Clawd" — the Claude Code terracotta mascot
  - "Disable the pet"

Write the choice into `plugins/claude-hud/config.json` (merge, same as Step 4): `pet.style: "claude"` for Clawd, `pet.enabled: false` to disable, nothing for the default cat.

**Petting**: inside Claude Code the user can run `/claude-hud:pet` (ships with the plugin, no install needed) — the pet reacts with a kawaii face.

**Optionally install the terminal `pet` command** (skip if the user disabled the pet, ask before installing). It does the same from any plain terminal:

**macOS/Linux** (and Windows + Git Bash):
```bash
mkdir -p "$HOME/bin"
cp "{PLUGIN_DIR}/scripts/pet" "$HOME/bin/pet"
chmod +x "$HOME/bin/pet"
command -v pet >/dev/null 2>&1 || echo 'NOTE: add $HOME/bin to PATH to use the `pet` command'
```

**Windows + PowerShell**:
```powershell
$claudeDir = if ($env:CLAUDE_CONFIG_DIR) { $env:CLAUDE_CONFIG_DIR } else { Join-Path $HOME ".claude" }
$petDir = Join-Path $claudeDir "plugins\claude-hud"
New-Item -ItemType Directory -Force $petDir | Out-Null
Copy-Item (Join-Path "{PLUGIN_DIR}" "scripts/pet.bat") (Join-Path $petDir "pet.bat") -Force
$userPath = [Environment]::GetEnvironmentVariable('Path','User')
if ($userPath -notlike "*claude-hud*") { [Environment]::SetEnvironmentVariable('Path', ($userPath.TrimEnd(';') + ';' + $petDir), 'User'); Write-Host "added to user PATH (new terminals only)" }
```

Tell the user: type `pet` in any terminal (new terminals on Windows) and watch the HUD.
...
2. Schedule the notifier every minute (skip if already present):
   ...
   **Windows** — per-user scheduled task (no Administrator needed; run it yourself, do not ask the user). Use `pythonw.exe` so no console window flashes; resolve it next to the detected `python.exe` and fall back to `python.exe` if missing:
   ```powershell
   $py = (Get-Command python).Source
   $pyw = Join-Path (Split-Path $py) "pythonw.exe"
   if (-not (Test-Path $pyw)) { $pyw = $py }
   $script = Join-Path $HOME ".claude\scripts\claude-notify.py"
   ...
   Register-ScheduledTask -TaskName "claude-hud-notify" -Action $action -Trigger $trigger -Settings $settings -Force | Out-Null
   ```
   Verify with `Get-ScheduledTask -TaskName "claude-hud-notify"` (State should be `Ready`), then fire a test notification:
   ```powershell
   python "$HOME\.claude\scripts\claude-notify.py" --test
   ```
...
**If yes**: Ask the user if they'd like to ⭐ star the claude-hud repository on GitHub to support the project. If they agree and `gh` CLI is available, first check whether their `gh` version supports `gh repo star`. If it does, run `gh repo star 501ego/claude-hud`. Otherwise fall back to `gh api -X PUT /user/starred/501ego/claude-hud`. Only run the star command if they explicitly say yes.
...
| `pet.enabled` | `boolean` | `true` | Animated pixel-art pet beside the HUD |
| `pet.style` | `'cat' \| 'claude'` | `'cat'` | Pet appearance (cat or Clawd, the CLI mascot) |
| `pet.position` | `'left' \| 'right'` | `'right'` | Which side of the HUD the pet anchors to |
| `pet.minWidth` | `number` | `80` | Hide the pet below this terminal width |
| `pet.rightMargin` | `number` | `8` | Columns kept free at the right edge |
| `pet.roamWidth` | `number` | `26` | Strip width (sprite + patrol room) |
| `pet.debug` | `boolean` | `false` | Append a width-diagnostic line |
